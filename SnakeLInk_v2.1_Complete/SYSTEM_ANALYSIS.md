...
4. **Link Discovery**: FSM evaluates signals; default to FSO (laser) as primary; fallback to Wi-Fi if FSO degrades
5. **Normal Operation**: Laptop sends H-Link packets via UDP; ESP32 routes via active link
6. **Degradation**: FSO ADC drops (fog/rain/misalignment) → FSM enters Warning → duplicate packets → switch to Wi-Fi
