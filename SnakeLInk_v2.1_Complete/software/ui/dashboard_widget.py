...
class DashboardWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # FSO (PRIMARY)
        grid.addWidget(QLabel("FSO Signal (PRIMARY):"), 0, 0)
        self.bar_fso = QProgressBar()
        self.bar_fso.setRange(0, 3300)
        self.bar_fso.setValue(1650)
        grid.addWidget(self.bar_fso, 0, 1)
        self.lbl_fso = QLabel("1.65V")
        grid.addWidget(self.lbl_fso, 0, 2)

        # Wi-Fi (BACKUP)
        grid.addWidget(QLabel("Wi-Fi RSSI (BACKUP):"), 1, 0)
        self.bar_wifi = QProgressBar()
        self.bar_wifi.setRange(-90, -30)
        self.bar_wifi.setValue(-80)
        grid.addWidget(self.bar_wifi, 1, 1)
        self.lbl_wifi = QLabel("-80 dBm")
        grid.addWidget(self.lbl_wifi, 1, 2)

        # LoRa (EMERGENCY)
        grid.addWidget(QLabel("LoRa RSSI (EMERGENCY):"), 2, 0)
        ...
        self.lbl_active = QLabel("Active: FSO")
        self.lbl_active.setStyleSheet("color: #e94560; font-weight: bold;")
        ...
        stats_layout.addWidget(self.lbl_active, 2, 0, 1, 2)
