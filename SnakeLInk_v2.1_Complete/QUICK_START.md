...
## First Use Checklist
- [ ] Both ESP32s powered on and showing "READY" on OLED (FSO active)
- [ ] Laser alignment verified (buzzer pitch = signal strength)
- [ ] Key fingerprints match on both OLEDs
- [ ] Laptop connected to transceiver Wi-Fi
- [ ] Software shows "TRANSCEIVER CONNECTED" + "FSO" in status bar
- [ ] Send test message in Chat tab (flows via laser by default)
- [ ] Verify delivery ticks (✓ → ✓✓)
- [ ] Test Wi-Fi fallback: block laser beam → watch switch to Wi-Fi
- [ ] Test LoRa fallback: block both → emergency mode active
